...
init([ReqID, From]) ->
    ThisNode = node(),
    IdxNode = {_, ThisNode} = basic_db_utils:random_index_from_node(ThisNode),
    SD = #state{
        req_id      = ReqID,
        from        = From,
        index_node  = IdxNode,
...
sync_response({ok, ReqID, RemoteNodeID, RemoteNodeClockBase, MissingObjects}, State) ->
    basic_db_vnode:sync_response( [State#state.index_node],
