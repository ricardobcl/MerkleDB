...
%% @doc Used by {@link basic_db_index_hashtree} to requeue a poke on
%%      build failure.
-spec requeue_poke(index()) -> ok.
requeue_poke(Index) ->
    gen_server:cast(?MODULE, {requeue_poke, Index}).

%% @doc Used by {@link basic_db_exchange_fsm} to inform the entropy
...
set_debug(Enabled) ->
    Modules = [basic_db_index_hashtree,
               basic_db_entropy_manager,
               basic_db_exchange_fsm],
    case Enabled of
        true ->
...
init([]) ->
    ...
    Limits = case app_helper:get_env(basic_db, aae_throttle_limits, []) of
                 [] ->
        ...
        {_, {ok, Tree}} ->
            case do_get_lock(exchange_remote, FsmPid, State) of
                {ok, State2} ->
                    ...
                    basic_db_index_hashtree:start_exchange_remote(FsmPid, From, IndexN, Tree),
                    {noreply, State2};
                {Reply, State2} ->
...
schedule_reset_build_tokens() ->
    {_, Reset} = app_helper:get_env(basic_db, anti_entropy_build_limit,
                                    ?DEFAULT_BUILD_LIMIT),
    erlang:send_after(Reset, self(), reset_build_tokens).

reset_build_tokens(State) ->
    {Tokens, _} = app_helper:get_env(basic_db, anti_entropy_build_limit,
                                     ?DEFAULT_BUILD_LIMIT),
    State#state{build_tokens=Tokens}.

-spec settings() -> {boolean(), proplists:proplist()}.
settings() ->
    case app_helper:get_env(basic_db, anti_entropy, {off, []}) of
        {on, Opts} ->
            {true, Opts};
        {off, Opts} ->
            {false, Opts};
        X ->
            lager:warning("Invalid setting for basic_db/anti_entropy: ~p", [X]),
            application:set_env(basic_db, anti_entropy, {off, []}),
            {false, []}
    end.

-spec maybe_reload_hashtrees(riak_core_ring(), state()) -> state().
maybe_reload_hashtrees(Ring, State) ->
    case lists:member(basic_db, riak_core_node_watcher:services(node())) of
        true ->
...
reload_hashtrees(Ring, State=#state{trees=Trees}) ->
    ...
    _ = [basic_db_vnode:request_hashtree_pid(Idx) || Idx <- MissingIdx],
    State.
...
add_hashtree_pid(false, _Index, Pid, State) ->
    basic_db_index_hashtree:stop(Pid),
    State;
...
do_get_lock(Type, Pid, State=#state{locks=Locks}) ->
    Concurrency = app_helper:get_env(basic_db,
...
maybe_poke_tree(State) ->
    ...
    basic_db_index_hashtree:poke(Tree),
    State2.
...
start_exchange(LocalVN, {RemoteIdx, IndexN}, Ring, State) ->
    ...
    try riak_core_ring:index_owner(Ring, RemoteIdx) of
        Owner ->
            ...
            DownNodes = Nodes -- riak_core_node_watcher:nodes(basic_db),
            case DownNodes of
                [] ->
                    RemoteVN = {RemoteIdx, Owner},
                    start_exchange(LocalVN, RemoteVN, IndexN, Ring, State);
                _ ->
                    {{basic_db_down, DownNodes}, State}
            end
    catch
        error:{badmatch,_} ->
        ...
        primary ->
            case orddict:find(LocalIdx, State#state.trees) of
                error ->
                    ...
                    %% exchange.
                    basic_db_vnode:request_hashtree_pid(LocalIdx),
                    State2 = requeue_exchange(LocalIdx, RemoteIdx, IndexN, State),
                    {not_built, State2};
                {ok, Tree} ->
                    case basic_db_exchange_fsm:start(LocalVN, RemoteVN,
                                                    IndexN, Tree, self()) of
                        {ok, FsmPid} ->
...
all_pairwise_exchanges(Index, Ring) ->
    LocalIndexN = basic_db_utils:responsible_preflists(Index, Ring),
    Sibs = basic_db_utils:preflist_siblings(Index),
    lists:flatmap(
      fun(RemoteIdx) ->
              RemoteIndexN = basic_db_utils:responsible_preflists(RemoteIdx, Ring),
...
get_aae_throttle() ->
    app_helper:get_env(basic_db, ?AAE_THROTTLE_ENV_KEY, 0).

set_aae_throttle(Milliseconds) when is_integer(Milliseconds), Milliseconds >= 0 ->
    application:set_env(basic_db, ?AAE_THROTTLE_ENV_KEY, Milliseconds).

get_aae_throttle_kill() ->
    case app_helper:get_env(basic_db, ?AAE_THROTTLE_KILL_ENV_KEY, undefined) of
        true ->
            true;
        _ ->
            false
...
set_aae_throttle_kill(Bool) when Bool == true; Bool == false ->
    application:set_env(basic_db, ?AAE_THROTTLE_KILL_ENV_KEY, Bool).

get_max_local_vnodeq() ->
    try
        {element(2,lists:keyfind(basic_db_vnodeq_max, 1, riak_core_stat:get_stats())), node()}
    catch _X:_Y ->
            %% This can fail locally if riak_core & basic_db haven't finished their setup.
            {0, node()}
    end.

get_aae_throttle_limits() ->
    %% init() should have already set a sane default, so the default below should never be used.
    app_helper:get_env(basic_db, aae_throttle_limits, [{-1, 0}]).
...
set_aae_throttle_limits(Limits) ->
    case {lists:keyfind(-1, 1, Limits),
        ...
        {{-1, _}, true} ->
            ...
            application:set_env(basic_db, aae_throttle_limits, Limits),
            ok;
        {Else1, Else2} ->
