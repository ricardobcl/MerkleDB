...
%% given partition. Each basic_db vnode spawns its own index_hashtree. These
...
%%      see {@link basic_db_entropy_manager:start_exchange_remote/3}
...
init([Index, VNPid, Opts]) ->
    case determine_data_root() of
        undefined ->
            case basic_db_entropy_manager:enabled() of
                true ->
                    lager:warning("Neither basic_db/anti_entropy_data_dir or "
                                  "riak_core/platform_data_dir are defined. "
                                  "Disabling active anti-entropy."),
                    basic_db_entropy_manager:disable();
                false ->
                    ok
            end,
            ignore;
...
determine_data_root() ->
    case application:get_env(basic_db, anti_entropy_data_dir) of
        {ok, EntropyRoot} ->
            EntropyRoot;
        undefined ->
            case application:get_env(riak_core, platform_data_dir) of
                {ok, PlatformRoot} ->
                    ...
                    lager:warning("Config basic_db/anti_entropy_data_dir is "
                                  "missing. Defaulting to: ~p", [Root]),
                    application:set_env(basic_db, anti_entropy_data_dir, Root),
                    Root;
                undefined ->
                    undefined
            end
...
fold_keys(Partition, Tree, _HasIndexTree) ->
    ...
    riak_core_vnode_master:sync_command({Partition, node()},
                                        Req,
                                        basic_db_vnode_master, infinity),
    ok.
...
maybe_throttle_build(RObjBin, Limit, Wait, Acc) ->
    ObjSize = byte_size(basic_db_utils:encode_kv(RObjBin)),
    Acc2 = Acc + ObjSize,
...
fold_fun(Tree, _HasIndexTree = false) ->
    ...
    fun(BinBKey, RObj, Acc) ->
            BKey = basic_db_utils:decode_kv(BinBKey),
...
object_fold_fun(Tree) ->
    fun(BKey={Bucket,Key}, RObj, BinBKey) ->
            IndexN = basic_db_utils:get_index_n({Bucket, Key}),
...
do_build_finished(State=#state{index=Index, built=_Pid}) ->
    ...
    basic_db_entropy_info:tree_built(Index, BuildTime),
...
expand_item(_Has2ITree, {object, BKey, RObj}, Others) ->
    IndexN = basic_db_utils:get_index_n(BKey),
...
expand_delete_item(_Has2ITree, {object, BKey}, Others) ->
    IndexN = basic_db_utils:get_index_n(BKey),
...
responsible_preflists(#state{index=Partition, use_2i=_Use2i}) ->
    % RP = basic_db_utils:responsible_preflists(Partition) ++
    % [?INDEX_2I_N || Use2i],
    % RP.
    basic_db_utils:responsible_preflists(Partition).
...
get_all_locks(Type, Index, Pid) ->
    case basic_db_entropy_manager:get_lock(Type, Pid) of
        ok ->
...
maybe_get_vnode_lock(rehash, _Partition, _Pid) ->
    ...
    ok;
maybe_get_vnode_lock(build, _Partition, _Pid) ->
...
%     case riak_core_bg_manager:use_bg_mgr(basic_db, aae_use_background_manager) of
%         true  ->
