...
init([LocalVN, RemoteVN, IndexN, LocalTree, Manager]) ->
    Timeout = app_helper:get_env(basic_db,
...
prepare_exchange(start_exchange, State=#state{remote=RemoteVN,
                                              index_n=IndexN}) ->
    case basic_db_entropy_manager:get_lock(exchange) of
        ok ->
            case basic_db_index_hashtree:get_lock(State#state.local_tree,
                                                 local_fsm) of
                ok ->
...
exchange_bucket(Tree, IndexN, Level, Bucket) ->
    basic_db_index_hashtree:exchange_bucket(IndexN, Level, Bucket, Tree).

%% @private
exchange_segment(Tree, IndexN, Segment) ->
    basic_db_index_hashtree:exchange_segment(IndexN, Segment, Tree).
...
update_request(Tree, {Index, _}, IndexN) ->
    as_event(fun() ->
                     case basic_db_index_hashtree:update(IndexN, Tree) of
                         ok ->
...
remote_exchange_request(RemoteVN, IndexN) ->
    FsmPid = self(),
    as_event(fun() ->
                     basic_db_entropy_manager:start_exchange_remote(RemoteVN,
                                                                   IndexN,
                                                                   FsmPid)
             end).
...
send_exchange_status(Status, #state{local=LocalVN,
                                    remote=RemoteVN,
                                    index_n=IndexN}) ->
    basic_db_entropy_manager:exchange_status(LocalVN, RemoteVN, IndexN, Status).
